module Bravo
  class Client
    attr_reader :soap_client, :base_imp, :total
    ...
    def initialize(attrs = {})
      ...
      @soap_client = Savon::Client.new(wsdl: Bravo.service_url)
    ...
    def exchange_rate
      ...
      response = soap_client.call(
        :fe_param_get_cotizacion,
        message: body.merge!({"MonId" => Bravo::MONEDAS[moneda][:codigo]})
      )
      response.body[:fe_param_get_cotizacion_response][:fe_param_get_cotizacion_result][:result_get][:mon_cotiz].to_f
    end
    ...
    def authorize
      setup_bill
      response = soap_client.call(:fecae_solicitar, message: body)
    ...
    def setup_bill
      ...
      fecaereq = {
        "FeCAEReq" => {
          "FeCabReq" => Bravo::Client.header(cbte_type),
          "FeDetReq" => {
            "FECAEDetRequest" => {
      ...
      unless concepto == 0
        detail.merge!({
          "FchServDesde" => fch_serv_desde || today,
    ...
    def next_bill_number
      resp = soap_client.call(:fe_comp_ultimo_autorizado,
        message: {"Auth" => Bravo.auth_hash, "PtoVta" => Bravo.sale_point, "CbteTipo" => cbte_type}
      )

      resp.body[:fe_comp_ultimo_autorizado_response][:fe_comp_ultimo_autorizado_result][:cbte_nro].to_i + 1
    end
